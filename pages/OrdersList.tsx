...
import { Clock, CheckCircle, XCircle, ShoppingBag, MapPin, Printer, CreditCard, Banknote, QrCode, MessageSquare, AlertTriangle, Hash, Truck, Utensils, User, Phone, ExternalLink } from 'lucide-react';
...
const OrdersList: React.FC<Props> = ({ orders, updateStatus, products, addOrder, settings }) => {
  ...
  const displayGroups = useMemo(() => {
    ...
    filtered.forEach(order => {
      const groupKey = (order.type === 'MESA' && order.tableNumber) 
        ...
        : (order.type === 'ENTREGA' || order.type === 'BALCAO' ? order.id : order.id);
  ...
  const openWhatsApp = (phone: string, name?: string) => {
      ...
      const text = encodeURIComponent(`Olá ${name || 'cliente'}, seu pedido da ${settings.storeName} já está em preparo/pronto!`);
    ...
    <div className="space-y-6">
      <style>{`
        @media print {
            ...
            #thermal-receipt { position: absolute; left: 0; top: 0; width: ${settings.thermalPrinterWidth || '80mm'}; padding: 4mm; background: #fff; font-family: 'Courier New', monospace; font-size: 10pt; }
        }
      `}</style>
        ...
        {displayGroups.map(group => (
          <div key={group.id} className="bg-white rounded-[2rem] p-6 shadow-sm border border-gray-100 flex flex-col hover:shadow-xl transition-all">
            <div className="flex justify-between items-start mb-4">
              <div>
                <span className={`px-3 py-1 rounded-full text-[9px] font-bold uppercase ${group.type === 'MESA' ? 'bg-blue-50 text-blue-600' : group.type === 'ENTREGA' ? 'bg-purple-50 text-purple-600' : 'bg-green-50 text-green-600'}`}>
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  <span className="text-xs font-mono text-gray-300">#{group.id}</span>
                </div>
              </div>
            ...
            {(group.customerName || group.customerPhone) && (
                <div className="mb-4 p-3 bg-gray-50 rounded-xl border border-gray-100">
                    {group.customerName && <div className="flex items-center gap-2 mb-1"><User size={14} className="text-gray-400"/> <span className="text-xs font-bold text-gray-700 uppercase">{group.customerName}</span></div>}
                    {group.customerPhone && (
                        <button onClick={() => openWhatsApp(group.customerPhone!, group.customerName)} className="flex items-center gap-2 text-green-600 hover:text-green-700 transition-colors">
            ...
            {group.deliveryAddress && (
              <div className="mb-4 p-3 bg-purple-50 rounded-xl border border-purple-100 flex items-start gap-2">
                  ...
                  <p className="text-[10px] text-purple-800 font-bold leading-tight uppercase">{group.deliveryAddress}</p>
              </div>
            )}

            <div className="flex-1 space-y-2 mb-6 max-h-48 overflow-y-auto px-1">
              {group.items.map((item, idx) => (
                <div key={idx} className="flex justify-between items-center text-sm border-b border-gray-50 pb-1">
                  <span className="text-gray-700"><strong className="text-orange-500 mr-1">{item.quantity}x</strong> {item.name}</span>
            ...
            <div className="border-t pt-4">
              <div className="flex justify-between items-end mb-4">
                 <div>
                      ...
                      <p className="text-2xl font-brand font-bold">R$ {group.total.toFixed(2)}</p>
                 </div>
              ...
              <div className="flex gap-2">
                  {group.status === 'PREPARANDO' ? (
                      ...
                      <button onClick={() => handleStatusUpdate(group, 'ENTREGUE')} className="flex-1 py-4 bg-green-600 text-white rounded-2xl font-bold text-xs hover:bg-green-700 shadow-lg flex items-center justify-center gap-2"><ShoppingBag size={16} /> Entregue / Finalizar</button>
                  )}
              </div>
            </div>
          </div>
      ...
      {printOrder && (
          <div id="thermal-receipt">
              ...
              <div style={{ borderTop: '1px dashed #000', padding: '2mm 0' }}>
                  ...
                  {printOrder.deliveryAddress && <p style={{ fontSize: '8pt' }}>END: {printOrder.deliveryAddress}</p>}
              </div>
              <div style={{ borderTop: '1px dashed #000', padding: '2mm 0' }}>
